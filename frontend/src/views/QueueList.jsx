...
const QueueList = (props) => {
    ...
    return (
        <Space direction="vertical" size={16}>
            ...
            {showering ? (
                <Card size="small" title={`(${showering["user_profile_id.gender"]}) ${showering["user_profile_id.name"]}`} style={{ width: 300 }}>
            ...
            {waiting.map((entry) => (
                <Card key={entry.id} size="small" title={`(${entry["user_profile_id.gender"]}) ${entry["user_profile_id.name"]}`} style={{ width: 300 }}>
